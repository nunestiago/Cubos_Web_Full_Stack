...
const cadastrarUsuario = async (req, res) => {
  ...
  if (senha.length < 5) {
    return res
      .status(404)
      .json("O campo senha deve conter, no mínimo, 5 caracteres");
  }
  ...
  try {
    ...
    return res.status(400).json("Usuário cadastrado com sucesso");
  } catch (error) {
  ...
  try {
    ...
    if (username !== req.usuario.username) {
      ...
      if (usernameUsuarioExiste) {
        return res.status(404).json("Usuário já existe");
      }
    }
